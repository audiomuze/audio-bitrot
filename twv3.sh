...
SUCCESS_LOG="/tmp/WV_${PWD_TAG}_successful"
CORRUPTED_LOG="/tmp/WV_${PWD_TAG}_corrupted"
DECODE_ERROR_LOG="/tmp/WV_${PWD_TAG}_decode_errors"
QUEUE_FILE="/tmp/WV_${PWD_TAG}_queue"
RUN_LIST="/tmp/WV_${PWD_TAG}_run_list"
NEXT_QUEUE_FILE="/tmp/WV_${PWD_TAG}_next_queue"
FIND_ERR_LOG="/tmp/WV_${PWD_TAG}_find_errors"
COUNTER="/tmp/WV_${PWD_TAG}_processed_count"
PROCESSED_FILES="/tmp/WV_${PWD_TAG}_processed_files"
LOCKFILE="/tmp/WV_${PWD_TAG}_lock"  # For atomic counter updates
PROCESSED_DB="/tmp/WV_${PWD_TAG}_processed_db"
APPEND_LOCKFILE="/tmp/WV_${PWD_TAG}_append_lock"
INSTANCE_LOCKFILE="/tmp/WV_${PWD_TAG}_instance_lock"
...
if [[ "$REFRESH" -eq 1 || ! -s "$QUEUE_FILE" ]]; then
    echo "Scanning for WavPack files..."
    if ! find . -type f -iname "*.wv" -print0 > "$QUEUE_FILE" 2> "$FIND_ERR_LOG"; then
...
process_file() {
    ...
    # Validate WavPack file (verify only, quiet mode), capturing errors but not printing to screen
    if "$WVUNPACK_BIN" -v -q "$file" >/dev/null 2>> "$DECODE_ERROR_LOG"; then
...
export WVUNPACK_BIN SUCCESS_LOG CORRUPTED_LOG DECODE_ERROR_LOG COUNTER PROCESSED_FILES PROCESSED_DB LOCKFILE APPEND_LOCKFILE
